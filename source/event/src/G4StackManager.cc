...
theMessenger = (G4StackingMessenger *)malloc(sizeof(G4StackingMessenger));
memset(theMessenger, 0, sizeof(G4StackingMessenger));
theMessenger = new (theMessenger)G4StackingMessenger(this);

#ifdef G4_USESMARTSTACK
//  urgentStack = new G4SmartTrackStack;
urgentStack = (G4SmartTrackStack *)malloc(sizeof(G4SmartTrackStack));
memset(urgentStack, 0, sizeof(G4SmartTrackStack));
urgentStack = new (urgentStack)G4SmartTrackStack;

 // G4cout<<"+++ G4StackManager uses G4SmartTrackStack. +++"<<G4endl;
#else
//  urgentStack = new G4TrackStack(5000);
urgentStack = (G4TrackStack *)malloc(sizeof(G4TrackStack));
memset(urgentStack, 0, sizeof(G4TrackStack));
urgentStack = new (urgentStack)G4TrackStack(5000);

//  G4cout<<"+++ G4StackManager uses ordinary G4TrackStack. +++"<<G4endl;
#endif
//  waitingStack = new G4TrackStack(1000);
waitingStack = (G4TrackStack *)malloc(sizeof(G4TrackStack));
memset(waitingStack, 0, sizeof(G4TrackStack));
waitingStack = new (waitingStack)G4TrackStack(1000);

//  postponeStack = new G4TrackStack(1000);
postponeStack = (G4TrackStack *)malloc(sizeof(G4TrackStack));
memset(postponeStack, 0, sizeof(G4TrackStack));
postponeStack = new (postponeStack)G4TrackStack(1000);
}
