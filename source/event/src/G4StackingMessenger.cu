...
stackDir = (G4UIdirectory *)malloc(sizeof(G4UIdirectory));
memset(stackDir, 0, sizeof(G4UIdirectory));
stackDir = new (stackDir)G4UIdirectory("/event/stack/");

  stackDir->SetGuidance("Stack control commands.");

//  statusCmd = new G4UIcmdWithoutParameter("/event/stack/status",this);
statusCmd = (G4UIcmdWithoutParameter *)malloc(sizeof(G4UIcmdWithoutParameter));
memset(statusCmd, 0, sizeof(G4UIcmdWithoutParameter));
statusCmd = new (statusCmd)G4UIcmdWithoutParameter("/event/stack/status",this);

  statusCmd->SetGuidance("List current status of the stack.");

// clearCmd = new G4UIcmdWithAnInteger("/event/stack/clear",this);
clearCmd = (G4UIcmdWithAnInteger *)malloc(sizeof(G4UIcmdWithAnInteger));
memset(clearCmd, 0, sizeof(G4UIcmdWithAnInteger));
clearCmd = new (clearCmd)G4UIcmdWithAnInteger("/event/stack/clear",this);
...
verboseCmd = new (verboseCmd)G4UIcmdWithAnInteger("/event/stack/verbose",this);
