...
/* Modify 2015/11/19 */
//   fStep = new G4Step();
fStep = (G4Step *)malloc(sizeof(G4Step));
memset(fStep, 0, sizeof(G4Step));
fStep = new (fStep)G4Step();
...
fVerbose = (G4SteppingVerbose *)malloc(sizeof(G4SteppingVerbose));
memset(fVerbose, 0, sizeof(G4SteppingVerbose));
fVerbose = new (fVerbose)G4SteppingVerbose();
